...
public class MainPanel extends JPanel {
  // members
	...
	private javax.swing.Timer winBlinkTimer;
  ...
  public final void restartGame()
  {
    ...
    if (winBlinkTimer != null) {
      ...
      winBlinkTimer = null;
		...
		stockPile = Util.toStockPile(deck.draw(52 - 28));
		allStacks.add(stockPile);
		drawn = new Drawn();
		allStacks.add(drawn);
  ...
  private void checkForWin()
  {
		for (int i = 0; i < homeStacks.size(); i++)
			if (homeStacks.get(i).size() != 13 || !gameRunning)
				return;
		winBlinkTimer = new javax.swing.Timer(500, new Blink());
		winBlinkTimer.start();
  ...
  @Override
	public void paintComponent(Graphics g) {
		...
		if (!stockPile.empty()) {
			stockPile.peek().drawAndPopulate(g);
		}
		if (!drawn.empty())
	...
	public void drawWin(Graphics g) {
		...
		Util.drawBitmap(Util.winMap, 100, 400, 20, g);
	}
    ...
    @Override
		public void mouseClicked(MouseEvent e) {
			if (e.getButton() == MouseEvent.BUTTON3) {
				returnHome();
			} else if (e.getX() >= 30 && e.getX() <= 130 && e.getY() >= 30
				         	&& e.getY() <= 210) {
				Solitaire.undoButton.newState();
				nextCard();
			}
